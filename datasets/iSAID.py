...
def make_dataset(quality, mode, hardnm=0):
    ...
    c_tokens.sort()
    mask_tokens = [c_token.replace(
        '.png', '_instance_color_RGB.png') for c_token in c_tokens]

    for img_token, mask_token in zip(c_tokens, mask_tokens):
        token = (osp.join(image_path, img_token),
                 osp.join(mask_path, mask_token))
        all_tokens.append(token)
    logging.info(
        f'iSAID has a total of {len(all_tokens)} images in {mode} phase')
...
class ISAIDDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image, mask = Image.open(image_path).convert(
            'RGB'), Image.open(mask_path)
