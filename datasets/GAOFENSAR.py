...
num_classes = 2
ignore_label = 255
root = cfg.DATASET.GAOFENSAR

label2trainid = {0: 0, 255: 1}
id2cat = {0: 'background', 1: 'Farm'}
...
def make_dataset(quality, mode, hardnm=0):
    ...
    c_tokens.sort()
    mask_tokens = [c_token for c_token in c_tokens]
    ...
    logging.info(f'GAOFENSAR has a total of {len(all_tokens)} images in {mode} phase')

    logging.info(f'GAOFENSAR-{mode}: {len(all_tokens)} images')

    return all_tokens


class GAOFENSAR(Dataset):
    ...
    def __init__(self, batch_size, shuffle, num_workers, quality, mode, maxSkip=0, joint_transform_list=None,
        ...
        super(GAOFENSAR, self).__init__(batch_size=batch_size,
